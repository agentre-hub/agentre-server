...
type DeviceReport struct {
	...
	// 这里刻意**没有** pending_backfill_days：服务端今天没有任何地方知道一台机器还差
	// 多少天的回填——ReportedThrough 给的是「上报到哪天」，从它减不出「还差几天」，
	// 因为拉取下界本身可以是「没有下界」。编一个数字出来比不给更糟。等数据层真有进度
	// 可交时，在这里加上它，前端那一格也一并加回去。
}

...
// 迁移里的建表语句**就是**这套 schema 的全部（0.1.0 首发是一条初始迁移）。于是
// 「实体上加了一个字段、迁移里没有对应列」不再是"下一次迁移会补上"，而是直接在第一次
// INSERT 上炸。这条守卫把两边对上：实体的每个列都要在 schema 里存在，建表语句的每张表
// 也都要有实体认领（有实体才谈得上漂移）。
//
// 守卫读的是**建表语句**。将来某条补丁迁移改动了列（加列、改名、删列），这里要跟着学会
// 重放那一类语句，否则守卫会对着建表时的旧形态比对——那时它给出的绿色是假的。
...
//
// 建表语句写在 raw string 里，所以这里读的是**源码文本**而不是运行结果：

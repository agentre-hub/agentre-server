...
func TestWins_GivenDifferentVersions_ThenHigherVersionWins(t *testing.T) {
	newer := &SyncObject{Version: 9, OriginFingerprint: "fp-1"}
	older := &SyncObject{Version: 8, OriginFingerprint: "fp-99"}
...
func TestWins_GivenEqualVersions_ThenSourceDeviceBreaksTie(t *testing.T) {
	a := &SyncObject{Version: 9, OriginFingerprint: "fp-5"}
	b := &SyncObject{Version: 9, OriginFingerprint: "fp-4"}
	assert.True(t, a.Wins(b))
	assert.False(t, b.Wins(a))
	// 全等时谁也赢不了对方——比较是全序的，不会两边都判胜。
	assert.False(t, a.Wins(&SyncObject{Version: 9, OriginFingerprint: "fp-5"}))
}

// 客户端提交的时间戳一律不参与胜负：时间戳更新的那一方版本更低时照样输。
func TestWins_GivenClientTimestamps_ThenTheyDoNotDecide(t *testing.T) {
	stale := &SyncObject{Version: 9, OriginFingerprint: "fp-1", SyncUpdatedAt: 1}
	fresh := &SyncObject{Version: 8, OriginFingerprint: "fp-1", SyncUpdatedAt: 1 << 40}

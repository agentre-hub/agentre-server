...
//
// 这个数字在 0.1.0 首发那一轮从 0.5.0 收回了起点：首发前的四次抬版本全部发生在产品发布
// 之前，没有一个装在用户手上的构建报过它们。理由与后果（0.2.0~0.5.0 会被重新用一遍，而
// 协议版本不像迁移 id 那样留台账）写在上游桌面仓 internal/pkg/wireversion 的同名常量注释
// 里，本仓只是跟着协议 module 的 pin 走。

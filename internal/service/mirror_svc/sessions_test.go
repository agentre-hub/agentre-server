...
// 加列迁移回填过的每一行。
func saved(userID int64, fingerprint, sessionID string) agent_session_entity.SessionSave {
	return agent_session_entity.SessionSave{
		UserID: userID, DeviceFingerprint: fingerprint, PeerFingerprint: fingerprint,
		PeerSessionID: sessionID, FollowedAt: 1000,
	}
}

// savedFromPeer 造一条**别的端发起、这台机器承载**的保存记录(web 控制台派发出去的
// 那些)。两个指纹不是同一个值,这正是身份键要分两列存的原因。
func savedFromPeer(userID int64, machine, initiator, sessionID string) agent_session_entity.SessionSave {
	row := saved(userID, machine, sessionID)
...
func (f *fakeSaves) Delete(
	_ context.Context, userID int64, peer, sessionID string,
) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	kept := f.rows[:0]
	for _, row := range f.rows {
		if row.UserID == userID && row.PeerFingerprint == peer && row.PeerSessionID == sessionID {
			continue
		}
...
func todo(userID int64, fingerprint, sessionID string) agent_session_entity.DeleteTodo {
	return agent_session_entity.DeleteTodo{UserID: userID, PeerFingerprint: fingerprint, PeerSessionID: sessionID}
}
...
func (f *fakeTodos) RemoveDeleteTodo(_ context.Context, userID int64, fingerprint, sessionID string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	kept := f.rows[:0]
	for _, row := range f.rows {
...
func TestBegin_SavedConversationOnOnlineMachine_StartsMirroringIt(t *testing.T) {
	...
	require.NoError(t, NewSessions(a.sup).Begin(context.Background(), testUserID, testMachine, "42"))
...
func TestBegin_WebDispatchedConversation_MirroredUnderItsInitiator(t *testing.T) {
	...
	rig := newResidentRig(t)
	newFakeSaves(savedFromPeer(testUserID, testMachine, browser, "42"))
	session := machineSession(42, "跑一下失败的测试")
	...
	assert.Equal(t, []int64{1, 2}, rig.store.rowSeqs(browser, "42"),
...
func TestBegin_MachineOffline_ReportsFailure(t *testing.T) {
	...
	err := NewSessions(a.sup).Begin(context.Background(), testUserID, testMachine, "42")
...
func TestPurge_ClearsStoredContentAndStopsMirroringThatConversation(t *testing.T) {
	rig := newResidentRig(t)
	newFakeSaves(saved(testUserID, testMachine, "42"))
	rig.peer.sessions = []*agentrewire.SessionSummary{machineSession(42, "要删掉的")}
	...
	require.NoError(t, NewSessions(a.sup).Purge(ctx, testUserID, testMachine, "42"))

	assert.Empty(t, rig.store.rowSeqs(testMachine, "42"), "转录要跟着一起清掉")
	assert.Empty(t, rig.store.summaryOf(testUserID, testMachine, "42"), "摘要也没了,索引里当场看不到它")
	a.net.emit(t, notification(42, 2, "删完还在说"))
	assert.Never(t, func() bool { return len(rig.store.rowSeqs(testMachine, "42")) > 0 },
...
func TestPurge_NothingStored_IsIdempotent(t *testing.T) {
	...
	require.NoError(t, NewSessions(a.sup).Purge(context.Background(), testUserID, testMachine, "42"))
}

// ── 把删除传到执行端 ────────────────────────────────────────────────────────

// Given 执行那条对话的机器在线;When 把删除传过去;Then 它收到 typed session.delete,
// 带着它自己那套会话标识,而且**不点名对端** —— 这条连接通到的就是它,点名会被拒。
func TestDeleteOnPeer_OnlineMachine_SendsSessionDelete(t *testing.T) {
	rig := newResidentRig(t)
	newFakeSaves()
	a := rig.replica(t, replicaA)

	require.NoError(t, NewSessions(a.sup).DeleteOnPeer(context.Background(), testUserID, testMachine, "42"))

	deletes := rig.peer.callsOf(agentrewire.RpcMethod_RPC_METHOD_SESSION_DELETE)
	require.Len(t, deletes, 1)
	p := deletes[0].(*agentrewire.SessionDeleteRequest)
	assert.Equal(t, int64(42), p.GetSessionId())
...
func TestDeleteOnPeer_MachineOffline_ReportsOffline(t *testing.T) {
	...
	err := NewSessions(a.sup).DeleteOnPeer(context.Background(), testUserID, testMachine, "42")
...
func TestPurgeMachineDeleteTodos_ClearsTodosOfThatMachineAndKeepsItsConversations(t *testing.T) {
	rig := newResidentRig(t)
	newFakeSaves(saved(testUserID, testMachine, "42"))
	rig.peer.sessions = []*agentrewire.SessionSummary{machineSession(42, "退役机器上的老对话")}
	rig.peer.journal[42] = []*agentrewire.JournaledNotification{journalRow(42, 1), journalRow(42, 2)}
	todos := newFakeTodos(
		todo(testUserID, testMachine, "43"),
		todo(testUserID, testMachine, "44"),
		todo(testUserID, "fp-other-machine", "7"),
	)
	...
	assert.Equal(t, []agent_session_entity.DeleteTodo{todo(testUserID, "fp-other-machine", "7")}, todos.pending())
	assert.Equal(t, []int64{1, 2}, rig.store.rowSeqs(testMachine, "42"), "撤销设备不动账号里的对话")
	assert.NotEmpty(t, rig.store.summaryOf(testUserID, testMachine, "42"), "索引里它还在,只是此后只读")
}
...
func TestReplayPendingDeletes_MachineBack_DeletesOnPeerAndClearsTodos(t *testing.T) {
	...
	newFakeSaves()
	todos := newFakeTodos(
		todo(testUserID, testMachine, "42"),
		todo(testUserID, testMachine, "43"),
		todo(testUserID, "fp-other-machine", "7"), // 那台还没回来
	)
	agent_session_repo.RegisterDeleteTodo(todos)
	a := rig.replica(t, replicaA)
	...
	for _, request := range rig.peer.callsOf(agentrewire.RpcMethod_RPC_METHOD_SESSION_DELETE) {
		...
		sent = append(sent, p.GetSessionId())
	}
	sort.Slice(sent, func(i, j int) bool { return sent[i] < sent[j] })
	assert.Equal(t, []int64{42, 43}, sent, "机器回来了,它欠的每一条删除都要补做")
	assert.Equal(t, []agent_session_entity.DeleteTodo{todo(testUserID, "fp-other-machine", "7")},
...
func TestReplayPendingDeletes_MachineStillOffline_KeepsTodos(t *testing.T) {
	rig := newResidentRig(t)
	newFakeSaves()
	todos := newFakeTodos(todo(testUserID, testMachine, "42"))
	agent_session_repo.RegisterDeleteTodo(todos)
	a := rig.replica(t, replicaA)
	a.net.setOnline(false)
	...
	assert.Equal(t, []agent_session_entity.DeleteTodo{todo(testUserID, testMachine, "42")}, todos.pending())
}

...
type DeleteTodoRepo interface {
	...
	// ListDeleteTodosByDevice returns every pending delete owed by one machine
	// on one account, for that machine's mirror client to execute when it comes
	// back online. Scoped by user_id as well as device_fingerprint: a read that
	// drops user_id could surface another account's todos for a fingerprint
	// value that collides across accounts.
	ListDeleteTodosByDevice(ctx context.Context, userID int64, deviceFingerprint string) ([]*agent_session_entity.DeleteTodo, error)
	// RemoveDeleteTodo clears one todo — the machine executed it, or a device
	...
	// RemoveDeleteTodosByDevice 清掉挂在一台机器上的全部待办。设备被撤销之后这些
	// 删除指令永远执行不了——那台机器已经不归这个账号管（决策 7），留着只是一堆
	// 对着谁都发不出去的指令。一条都没有时是 no-op。
	RemoveDeleteTodosByDevice(ctx context.Context, userID int64, deviceFingerprint string) error
	// ListPendingMachines 交出全库范围内还欠着删除的机器，按
	// (user_id, device_fingerprint) 去重——巡检据此知道该找谁补做。
	//
...
func (r *deleteTodoRepo) ListDeleteTodosByDevice(
	...
	if err := db.Ctx(ctx).Where("user_id=? AND device_fingerprint=?", userID, deviceFingerprint).
...
// RemoveDeleteTodosByDevice 按账号 + 这台机器圈定：别的机器上的待办一条都不能碰，
// 而少了 user_id 会清掉别的账号压在同一个指纹值上的待办（指纹跨账号可以重复）。
func (r *deleteTodoRepo) RemoveDeleteTodosByDevice(
	ctx context.Context, userID int64, deviceFingerprint string,
) error {
	return db.Ctx(ctx).Where(
		"user_id=? AND device_fingerprint=?", userID, deviceFingerprint,
	).Delete(&agent_session_entity.DeleteTodo{}).Error
}

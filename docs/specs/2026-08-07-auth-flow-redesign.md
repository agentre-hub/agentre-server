...
**登录（画板 02–05）。** 标题、一句说明（GitHub 账号登录，桌面应用与 agentred 守护进程都经由它授权）、GitHub 登录按钮、一行服务条款脚注。当 URL 带 `user_code` 时，说明与按钮之间插入一块 `primary-soft` 的上下文条：一行小字说明「登录后继续授权设备」，下面是等宽的设备码。`next` 与 `user_code` 仍照现有方式透传给 authorize 端点。

**输入设备码（画板 06–09）。** 眉标「设备授权」、H1、一句说明、六个独立的码格（第三与第四格之间是一个分隔符，不是可输入格）、一行提示「不区分大小写，代码中不含 0、O、1、I」、主按钮、一行「没有看到代码？回到设备上重新发起授权」脚注。
...
**授权请求已过期**（`ApiError.code === 30202`，或确认屏倒计时归零）：进入 `/device/expired`。该屏给出「授权请求已过期」、一句「设备码有效期有限。回到设备上重新发起授权，然后输入新的代码。」，以及一个「重新输入代码」的主按钮返回 `/device`。

**已拒绝授权**：用户点「拒绝」且请求成功后进入 `/device/denied`。该屏给出「已拒绝授权」、一句「这台设备不会拿到任何访问权限。可以直接关闭页面。」，以及一个「关闭页面」的次级按钮。这一屏是中性收尾，不使用 `destructive` 色。

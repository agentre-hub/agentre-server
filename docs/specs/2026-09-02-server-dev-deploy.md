...
目标机上不再按 tag 累积 dev 镜像：`agentre-server:dev` 每次重打同一个 tag，containerd 的镜像存储不会留下悬空镜像（已核实）。
...
- **反向代理与 HTTPS** —— `/root/agentre-caddy/Caddyfile`（`coding.local` → 127.0.0.1:8443，internal TLS）存在但机器上没装 caddy（已核实）。dev 继续以明文 8443 直接对外，Cookie 的 `Secure` 属性由 `server.public_url` 的 `http` scheme 自动关闭。

...
//
// 2026-09-10 又做了一次同样的事，为 0.1.0 首发收口：把两条补丁折进它们各自改的那几张
// 基线建表语句，两条补丁整条退役：
//
//   - 202609070101（journal 的 payload 从 longblob 改成 json）→ 折进 202609040108
//   - 202609080101（八张表补 id 自增主键、原主键降级为唯一键）→ 折进 202609040105、
//     202609040106、202609040108、202609040110
//
// 折法就是「把终态直接写进建表语句」：补的列直接建在表上（且按终态列序放在首位），
// 改过的列型直接写成终态，原来的主键原样降级成 UNIQUE KEY。等价性不是推的：同一份
// mysql:9.7.2 上分别跑旧 12 条与新 10 条，逐表 SHOW CREATE TABLE 完全一致。
//

...
/**
 * 审计页（Pencil 正式画板 bKvB4 的信息层级：标题 → 筛选行 → 事件表卡 → 右列凭证卡）。
 *
 * 审计后端本轮不存在（范围外），页面只呈现正式层级 + 诚实空态，不伪造：
 *   - 事件表、凭证区都用共享 EmptyState 表示「暂无数据」——不显示示例事件/IP/令牌/时间；
...
export default function Audit() {
  ...
  return (
    <AppShell title={t("nav.audit")}>
      <div data-testid="audit-body" className="flex flex-col gap-4 lg:flex-row">
        {/* 左：事件表区 */}

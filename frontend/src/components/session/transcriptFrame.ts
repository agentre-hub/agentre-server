 ...
 * 说得清「同一号只画一次」。这一道闸门**不只管持久帧**：轮次结束派生的 error / done /
 * context_window_updated 也带着**来源终态通知**的 seq（见 turnDone），同一个终态派
 * 生出的多帧共享同一个号 —— 于是同一批一起来时整批通过，重放那批再一起来时整批
 * 被拦，不会被截成半批。真正无号可判的（seq 0 / 缺省）照单收下：它们不占持久帧的
 * 号，也无从判断是不是同一条，宁可重画也不能误吞。
 */
